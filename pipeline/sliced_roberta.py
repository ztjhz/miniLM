...
def main():
    ...
    # set up model
    with open("ds_config_roberta.json", "r") as f:
        df_config = json.load(f)
    model = SlicedRoberta(num_labels=num_labels)
    model_engine, _, _, _ = deepspeed.initialize(model=model,
                                                model_parameters=model.parameters(),
                                                config=df_config)

    tokenized_datasets = tokenize(dataset, "roberta-base")
    ...
    run_name = f"Sliced Roberta - {args.dataset}"
